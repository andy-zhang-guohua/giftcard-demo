...
- 观察到的现象
    - Axon服务器，应用启动后，
        - 如果删除数据库表(业务实体)中的数据，或者删除数据库表(业务实体)，重启应用/Axon服务器都不会导致数据表(业务实体)中数据重建;
            - 也就是说此时Axon服务器上的事件不会在数据库上重放形成数据
            - 如果这种情况下也要重发形成业务实体表数据,需要对表 token_entry 做相应的处理(清空)
        - 如果删掉整个数据库,重启应用会导致数据表(业务实体)重建，数据表(业务实体)中的数据也会重建;
